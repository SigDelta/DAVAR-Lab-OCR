...
model = dict(
    ...
    test_cfg=dict(
        ...
        postprocess=dict(type="PostLGPMA"),
    ),
)
...
val_pipeline = [
    ...
    dict(
        ...
        transforms=[
            ...
            dict(type="DavarDefaultFormatBundle"),
            dict(
                type="DavarCollect", keys=["img", "gt_bboxes", "gt_labels", "gt_masks"]
            ),
        ],
    ),
]
...
data = dict(
    samples_per_gpu=3,
    workers_per_gpu=1,
    train=dict(type=dataset_type, ann_file="", img_prefix="", pipeline=train_pipeline),
    val=dict(type=dataset_type, ann_file="", img_prefix="", pipeline=val_pipeline),
    test=dict(type=dataset_type, ann_file="", img_prefix="", pipeline=test_pipeline),
)
